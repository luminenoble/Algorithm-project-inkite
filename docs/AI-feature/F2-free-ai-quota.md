...
> **目标**：用户填入 3 个自定义关键词召唤 AI 折纸；这 3 词织进 Replicate prompt
> 并随 origami 文档落库；周配额限制成本与使用频次；交互愈深 bonus 越多。
> **状态**：新增 Callable + 新增 stats / aiUsage 字段 + origami.words 字段，
> **不破坏现有 generateOrigami / 官方挑战流**

> ⚠️ 后续更新（v2）：把「随意点按钮生成」改为「必须输入 3 词」。
> 改动点：CF 验证 words 数组（长度=3、单词≤16 字、非空）；客户端 3 词表单；
> origami 文档新增 `words` 字段。下文规则已是最新版。
...
- 顶部右侧 `已用 / 上限` 实时显示
- 三个输入框：必须全填、单词 ≤ 16 字，否则按钮置灰 + 行下方红字提示
- 中间两个 chip 显示 bonus 状态：解锁的金箔色 + 勾、未解锁灰色 + 锁
- 「召唤」按钮调 CF；成功后清空输入框便于下次召唤；配额耗尽时置灰
